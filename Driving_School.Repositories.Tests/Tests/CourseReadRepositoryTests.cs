...
namespace Driving_School.Repositories.Tests.Tests
{
    ...
    public class CourseReadRepositoryTests : Driving_SchoolContextInMemory
    {
        ...
        /// <summary>
        /// Поиск курса в коллекции по идентификатору (true)
        /// </summary>
        [Fact]
        public async Task IsNotNullEntityReturnTrue()
        {
            //Arrange
            var target1 = TestDataGenerator.Course();
            await Context.Courses.AddAsync(target1);
            await Context.SaveChangesAsync(CancellationToken);

            // Act
            var result = await courseReadRepository.AnyByIdAsync(target1.Id, CancellationToken);
        ...
        /// <summary>
        /// Поиск курса в коллекции по идентификатору (false)
        /// </summary>
        [Fact]
        public async Task IsNotNullEntityReturnFalse()
        {
            ...
            // Act
            var result = await courseReadRepository.AnyByIdAsync(target1, CancellationToken);
        ...
        /// <summary>
        /// Поиск удаленного курса в коллекции по идентификатору
        /// </summary>
        [Fact]
        public async Task IsNotNullDeletedEntityReturnFalse()
        {
            //Arrange
            var target1 = TestDataGenerator.Course(x => x.DeletedAt = DateTimeOffset.UtcNow);
            await Context.Courses.AddAsync(target1);
            await Context.SaveChangesAsync(CancellationToken);
